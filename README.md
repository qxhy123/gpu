...
- **Persistent kernels** — `WorkQueue` (push/pop/stop/is_empty/is_stopped) + `PersistentKernel.start()` drains queue; `is_persistent=True` KernelLaunch events; `persistent_kernel_throughput` metric — `examples/persistent_kernel_server/` + `examples/persistent_work_queue/` + Chapters 54–55
- **Dynamic parallelism** — `device_launch(parent_kernel_id, ...)` schedules child kernels; `drain_pending_child_launches()` executes them; `parent_kernel_id` + `is_persistent` fields on `KernelLaunch`; `dynamic_parallelism_depth` + `dynamic_parallelism_fanout` metrics — `examples/dynamic_parallelism_recursive/` + Chapter 56
- **Persistent pipeline** ⭐ — producer + consumer `PersistentKernel` sharing a `WorkQueue`; capstone producer-consumer pattern — `examples/persistent_pipeline/` + Chapter 57
...
| **54** | **持久化内核服务器模式 — WorkQueue + PersistentKernel.start() + is_persistent 事件 + persistent_kernel_throughput** |
| **55** | **队列驱动的持久化处理 — 批量 push + stop + 多阶段 WorkQueue 增长** |
| **56** | **动态并行 — device_launch + drain_pending_child_launches + parent→child 调用链 + dynamic_parallelism_depth/fanout** |
| **57** | **持久化流水线 ⭐ — 生产者-消费者 PersistentKernel 共享 WorkQueue 的 capstone 示例** |
